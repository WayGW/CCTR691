...
> Red Flag 1: We have too many cells (17k)

> Red Flag 2: We have too few mea nreads per cell (well below 10k)
...
> orig.ident, nCount_RNA, and nFeature_RNA
...
> 1 sample (whim2), samples in the summary means number of cells (2575), 19,612 genes
...
> Many of the high percent.mt cells had very low ncounts and nfeatures
...
> fewer differences/distinctions are identified between cell groups (cells gene experssion are more similar)
...
> axis change and the plot rotates a bit, not much change in clustering to my eye

How about only the first dimension? 

> It turns into nondescript squiqqly worms

What happens when you use more dimensions?

> Clustering might be a little better but not by much (diminishing returns?)
...
VlnPlot(whim_filtered, features = c("CD5L","RRM2","NDRG1"), group.by = "seurat_clusters",ncol = 3, pt.size = .4)
```
...
FeaturePlot(whim_filtered, features = "percent.mt", reduction = "tsne", cols =c("grey","red"))
```
